    ...
    <section className="min-h-fit px-10 bg-stone-200 text-white pt-5 pb-2">
        ...
        <div className="px-5 w-full place-content-center items-stretch">
            <div className="place-self-center grid-cols-1 place-items-center">
                <p className="text-4xl lg:text-4xl text-gray-800 mb-3 justify-center">{text} </p>
            </div>
        </div>
        
        <div className="px-5 space-x-4 flex flex-row pt-10 justify-between pb-10">
            {product.map((pro, index) => (
                <div className="max-w-sm w-full bg-white border border-gray-200 rounded-lg 
                shadow dark:bg-gray-800 dark:border-gray-700" key={index} >
                    ...
                    <div className="flex justify-between">
                        <Link href={'/product/' + pro.id}>
                            <h5 className="px-3 pt-2 text-lg tracking-tight text-gray-900 dark:text-white hover:text-amber-800">
                            {pro.title}</h5>
                        </Link>
                    
                        {pro.discount ? <p className="text-lg pt-2 px-3 text-black
                        "><span className="pr-2 line-through pr-1 text-red-900">${pro.price} 
                        </span>${Math.round(pro.price -((pro.discount/100)* pro.price))}</p> : 
                        <p className="text-lg pt-2 px-3 text-black
                        ">${pro.price}</p>
                        }
                    ...
                    <div className="flex justify-between">
                        <h5 className="pb-2 px-3 text-md tracking-tight text-gray-900 dark:text-white">
